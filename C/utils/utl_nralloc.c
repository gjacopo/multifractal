/* nrutil.c
 *
 * Routines for memory allocation of 1-dim and 2-dim arrays
 *
 * Taken from Numerical Recipes, 2nd edition, Appendix B
 *
 * 5-25-98
 *
 */
...
/* allocate an unsigned double vector with subscript range v[nl..nh] */
{
   double *v;
...
/* This routine added by Jon Hamkins -- need UNSIGNED int */
 ...
 * nrow=nrh-nrl+1 and ncol=nch-ncl+1.  The routine should be called with
 * the address &a[0][0] as the first argument. */
{
   ...
   /* allocate pointers to rows */
   m=(float **) malloc((size_t) ((nrow+NR_END)*sizeof(float*)));
   if (!m) nrerror("allocation faiure in convert_matrix()");
   m += NR_END;
   m -= nrl;
...
/* allocate a float 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
   ...
   /* allocate pointers to pointers to row */
...
/* free an int vector allocated with cvector() */
{
   free((FREE_ARG) (v+nl-NR_END));
}
